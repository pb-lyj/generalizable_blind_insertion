...
from resultant_mlp import create_tactile_policy_mlp, compute_mlp_policy_losses, prepare_mlp_input_from_flexible_dataset
...
def train_mlp_policy(config):
    ...
    output_dir = os.path.join(config['output']['output_dir'], f"mlp_policy_{timestamp}")
    ...
    run = wandb.init(
        ...
        config=config,
        dir=output_dir,
        tags=['mlp-policy', 'simplified'] + [timestamp],
        notes='Simplified MLP policy training'
    )
    ...
    print("=" * 60)
    print("MLP Policy Training (Simplified)")
    ...
    # 创建数据集
    print("📂 加载数据集...")
    prediction_step = config['data'].get('prediction_step', 1)
    print(f"📊 预测步长: t+{prediction_step}")
    train_dataset, test_dataset, normalization_params = create_classic_datasets(
    ...
    # 创建模型
    print("🏗️ 创建模型...")
    model = create_tactile_policy_mlp(config['model']).to(device)
...
def train_epoch(model, train_loader, optimizer, device, dataset=None):
    ...
    for batch_idx, batch in enumerate(tqdm(train_loader, desc="Training")):
        ...
        outputs = model(resultant_force, resultant_moment, current_action)
        
        # 计算损失
        loss, metrics = compute_mlp_policy_losses(mlp_inputs, outputs, dataset=dataset)
...
if __name__ == '__main__':
    ...
    config = {
        'data': {
            ...
            'prediction_step': 1,  # t+n 预测步长
        },
        'model': {
            'input_dim': 15,  # force(6) + moment(6) + current_action(3)
            'output_dim': 3,
            'hidden_dim': 128,
            'num_layers': 4,
            'dropout': 0.1
